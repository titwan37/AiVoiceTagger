...
# 31. Technical Stack & Current Improvement Strategies

To maximize the throughput and resilience of the architecture, several core improvement strategies have been formalized and adopted into the technology stack.

## 31.1 Technical Stack Enhancements

* **Rust Edge Core:**
  * **tokio**: Async orchestration and non-blocking I/O.
  * **symphonia / rodio**: Audio decoding, probing, and RMS peak energy detection for smart snippet selection.
  * **whisper-rs**: Heavy STT processing pool.
  * **rusqlite**: Shared SQLite state store with `WAL` mode and atomic lease transactions.
  * **tempfile**: Atomic lockfile creation (`.lock`) and guarded cleanup.
* **Python AI & Data Pipeline:**
  * **Pydantic v2**: Type-safe payload validation.
  * **spaCy / re**: NLP parsing and verbatim watchlist matching.
  * **Polars**: High-performance columnar aggregation and multi-format export (JSON/CSV/Parquet).
* **Scripting Utilities:**
  * **PowerShell**: `split_manifest.ps1` for round-robin offline manifest partitioning.
  * **Python**: `merge_dbs.py` for offline deterministic database merging.

## 31.2 Current Improvement Strategies

### 1. Three-Stage Triage Engine (TinySorting)
To eliminate wasting 100% of expensive Whisper compute on 100% of raw audio chunks, a fast-pass triage mechanism has been implemented:
* **Smart Snippet Extraction:** Instead of processing entire files, `decoder.rs` extracts the first 30 seconds, last 30 seconds, and the 30-second window containing the highest RMS audio energy.
* **Tiny Quantized Model:** A secondary lightweight Whisper pool (`ggml-tiny-q8_0.bin` at `beam_size=1`) rapidly transcribes these snippets at ~20x real-time speed.
* **Watchlist Matching:** Transcriptions are immediately checked against legal/harassment watchlists. Matches are promoted to `TriagedHighInterest` for full deep-dive transcription, yielding an ~85% reduction in total CPU compute time across large collections.

### 2. 3-Layer Defense-in-Depth Concurrency Control (Lock-Free)
A decentralized, zero-collision concurrency model for running multiple worker nodes (`pc-alpha`, `pc-beta`) on the same `\\SyNAS\Records` share:
1. **Layer 3 - Manifest Partitioning:** `split_manifest.ps1` partitions the total workload offline via round-robin distribution so workers naturally avoid each other.
2. **Layer 2 - Network Sidecar Lockfiles (`.lock`):** Workers attempt to create an atomic `audio.wav.lock` sidecar file containing their ID and a timestamp. Locks older than 30 minutes are treated as stale (from a crashed worker) and reclaimed.
3. **Layer 1 - Shared SQLite WAL Leases:** A centralized SQLite database operating in `WAL` mode with a `10000ms` busy timeout issues atomic leases (`UPDATE records SET lease_owner = 'pc-alpha' ... RETURNING *`).

### 3. High-Priority Folder Triage
The batch scanning process has been optimized to handle critical assets immediately before touching archive directories:
* **Dedicated High-Priority Paths:** Assets in `\RecordStrike` and `\Audio\Select_Sort` are dynamically boosted to the front of the queue by the manifest partitioner.
* **Format Hierarchy:** Pure audio assets (`.wav`, `.mp3`) always take precedence over video assets (`.mp4`, `.mkv`).
* **Scrambling:** Remaining items are scrambled across target years (2019-2024) to ensure rapid uniform temporal coverage before full completion.
